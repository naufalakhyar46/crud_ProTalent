...
const getData = async () => {
    ApiService.query("/user/show/" + route.params.id)
        .then((res) => {
            let data = res.data.data.data;
            model.value.name = data.name
            model.value.email = data.email
            model.value.path = data.path
        })
}
...
const saveData = async () => {
    ...
    ApiService.post('/user/update/' + route.params.id, formData)
        .then((res) => {
            Swal.fire({
...
<template>
    ...
    <div style="margin-top:50px" class="max-w-xl mx-auto">
        ...
        <h2 class="text-4xl font-extrabold mb-6 mt-3">Edit User</h2>
        <form ref="refs" @submit.prevent="saveData">
            ...
            <div class="w-full mb-5 group">
                ...
                <fwb-input type="file" label="Image" size="sm" @change="upload" />
            </div>
            <button type="submit"
